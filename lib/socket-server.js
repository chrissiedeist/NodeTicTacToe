...
var alertMove = function(socket, io){
  ...
  // don't alert move if only one player has entered (guestNumber determines the number
  // of the next guest)
    if (guestNumber === 2) {
      var message = "Please wait until the other player's connection has been established"
      io.sockets.emit('move_attempt', { message: message })
    }else {
